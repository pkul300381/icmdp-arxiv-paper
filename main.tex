...
cumulative costs. A nonnegative violation cost with zero budget can encode zero
violation occupancy along induced trajectories; Section~\ref{subsec:optimality}
formalizes this relationship.

We present an \emph{Intent-Constrained Markov Decision Process} (ICMDP), in
...
executed-policy probability to actions excluded by the active compiled
specification. We give the restricted Bellman equations and
feasibility conditions,
record the standard contraction property inherited from
state-dependent-action MDPs, and characterize the relationship to a
zero-budget CMDP encoding.
The principal contribution is a language-to-admissibility framework in
...
changing intent, and computational cost.
An autonomous cloud-operations example follows a deployment restriction
...
Positive budgets permit nonzero expected
...
enforce specified action-level constraints at each step.
These
...
established mechanisms. We do not claim novelty for restricting
optimization to actions available in the current state.
Subject to the detailed comparison with related work, the
...
The resulting policy assigns zero executed-policy probability to actions
excluded by the active compiled specification, provided that the specification,
state information, and enforcement mechanism are correct.
...
For a trajectory generated by policy $\pi$, define the discounted
return from time $t$ as
\[
  G_t \coloneqq \sum_{k=0}^{\infty}\gamma^k
  R(s_{t+k},a_{t+k}).
\]
The state-value function is the expected return conditional on starting
in state $s$:
\begin{equation}
...
\]
This decomposition separates the immediate reward from the discounted
...
\begin{equation}
  ...
  \label{eq:bellman}
\end{equation}
Equation~\eqref{eq:bellman} gives the optimal value, while a maximizing
...
\begin{equation}
  ...
  \label{eq:lagrangian}
\end{equation}
Each nonnegative multiplier $\lambda_k$ acts as a shadow price on
...
The Lagrangian is an optimization device for solving the constrained
...
For a nonnegative violation cost, a positive CMDP budget permits
nonzero expected discounted violation cost and therefore does not, in general,
imply that violations occur with probability zero. By contrast, when the
...
therefore not that CMDPs are incapable of representing zero executed-policy
mass on designated actions. Rather, ICMDP makes the statewise admissible-action set explicit
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[box] (spec) at (5,0) {Structured specification $G$};
    \node[box] (valid) at (7.5,0) {Schema, grounding, and consistency checks};
  ...
  \end{tikzpicture}
  \caption{Language-to-admissibility architecture. The LLM produces a
  structured specification outside the fast decision loop. The deterministic
  compiler creates a versioned executable rule once; planning and runtime
  enforcement then evaluate that rule on hypothetical or current states.
...
\begin{enumerate}[leftmargin=*,label=\arabic*.,nosep]
  ...
  $G_{\mathrm{new}}\leftarrow f_\phi(l,x)$.
  \item Validate syntax, types, grounding, consistency, and declared conflict
  semantics; retain the last validated rule but remain on hold and request
  clarification or escalation if validation fails.
  \item Compile once:
  ...
  activate $\cI^{(v+1)}\leftarrow\cI_{\mathrm{new}}$; do not silently relax an
  empty set.
...
Let $l \in \cL$ denote a natural-language instruction, let $x\in\cX$ denote
the grounding context, and let $\mathcal{G}$ be a typed intent-specification
language. Define the space of state-evaluable intent rules as
\[
  \mathfrak I
  \coloneqq
...
\begin{definition}[Intent parsing and compilation]
An \emph{LLM-based intent parser} and a deterministic \emph{intent compiler}
are mappings
\begin{equation}
  f_\phi : \cL \times \cX \to \mathcal{G},
  \qquad
  c : \mathcal{G}\to\mathfrak I,
  \qquad
  \cI_{l,x}=c\!\left(f_\phi(l,x)\right),
  \label{eq:llm_map}
\end{equation}
where $\phi$ denotes the parser parameters. The parser produces a structured
specification once, and the compiler converts it once into the reusable
function $\cI_{l,x}$. Planning and runtime enforcement subsequently evaluate
$\cI_{l,x}(s)$ at hypothetical or observed states.
\end{definition}

Compilation produces a general rule or predicate, not merely a mask over
...
For the cloud instruction in Example~\ref{ex:cloud}, the parser may
...
\end{equation}
The artifact $G$ denotes the general predicate in
...
functions. Constraint-aware planning
...
This paper presents ICMDP as a
...
Bellman equations and feasibility conditions, stated the standard
...
restricted action set. Statewise exclusions also admit a zero-budget CMDP
encoding (Proposition~\ref{prop:cmdp_encoding}); ICMDP's distinction is its
explicit lifecycle from language through
