...
cumulative costs. A zero-budget CMDP can encode zero executed-policy mass on
actions excluded by a specification along induced trajectories, but the user's
instruction must first be represented as costs and thresholds.
...
runtime overhead
in a reproducible simulated cloud-control MDP.}
\end{abstract}
...
A fixed intent function $\cI$ may return different permitted sets in
...
on zero intent-exclusion occupancy alone: relative to the same baseline
availability process, statewise intent exclusions can be encoded as a CMDP
constraint along trajectories reachable from a specified initial distribution.
The two representations nevertheless emphasize different
...
deterministic compilation algorithm $c$ with conformance tests linking each
...
conditions for masked Q-learning; Proposition~\ref{thm:contraction} establishes
...
An exogenous change to $\cI_t$ breaks the stationarity assumption
...
Constraint-aware planning and runtime enforcement serve different
...
deployed system. In particular, the
cloud-operations scenario in Example~\ref{ex:cloud} is an explanatory
...
runtime enforcement in a small simulated cloud-control MDP with
...
The cloud-operations example demonstrates this lifecycle with one fixed
